...
SELECT m.match_id,
       ...
       CASE 
           WHEN (m.home_team_id = '{team_id}' AND m.home_score > m.away_score) OR 
                (m.away_team_id = '{team_id}' AND m.away_score > m.home_score) THEN 1
           WHEN (m.home_team_id = '{team_id}' AND m.home_score < m.away_score) OR 
                (m.away_team_id = '{team_id}' AND m.away_score < m.home_score) THEN 2
           ELSE 3
       END as result_order,
       ABS(m.home_score - m.away_score) as score_diff
FROM matches m
LEFT JOIN teams ht ON m.home_team_id = ht.team_id
LEFT JOIN teams at ON m.away_team_id = at.team_id
WHERE m.home_team_id = '{team_id}'
   OR m.away_team_id = '{team_id}'
ORDER BY result_order, score_diff DESC;
"""
