...
if player_df.empty:
    ...
    if second_half_df.empty:
        ...
        # Geef de eerste contour van de heatmap een label, zodat deze in de legenda kan
        if heatmap_set.collections:
            heatmap_set.collections[0].set_label("Heatmap")
    
        # 2. Spelerposities (zwarte stippen)
        ...
        # 3. Startpunt (eerste rij in second_half_df) met rode marker
        ...
        # 4. Dead zones (gebieden met lage dichtheid)
        ...
        # Stel het 20e percentiel in als drempel voor 'dead zones'
        deadzone_threshold = np.percentile(Z, 50)  # in plaats van 20


        # Plot contourf voor de dead zones
        ...
        # We maken een legend-handle voor de dead zones
        deadzone_patch = mpatches.Patch(color='lightblue', alpha=0.4, label='Dead Zones')
    
        # Maak een legenda met duidelijke labels
        ax.legend(
            handles=[
        ...
        ax.set_title(f'Positie Heatmap voor speler B. Mechele (2e helft, >45 min) in game Club Brugge VS Anderlecht', fontsize=16)
        plt.tight_layout()
        plt.show()
